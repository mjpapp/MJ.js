define(function(require, ex){
	...
	// 数组转换为对象
	ex.toObject = function(arr, key, value){
		var obj = {};
		arr = arr || [];
		key = key || 'id';
		var isAll = value === true; // 返回全部
		value = value || 'name';

		if(arr.length){
			for(var i=0; i<arr.length; i++){

				if(isObject(arr[i]) && arr[i].hasOwnProperty(key)){
					obj[arr[i][key]] = isAll ? arr[i] : arr[i][value];
				}else{
					if (arr[i] !== undefined){
						obj[i] = arr[i];
					}

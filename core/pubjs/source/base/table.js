define(function(require, exports){
		...
		afterBuild: function(){
			...
			foot.append([
				...
				'<div class="checkboxCon">',
					'<button class="btn btn-default" data-action="all">全选</button>',
					'<button class="btn btn-default" data-action="invert">反选</button>',
				'</div>'
			].join(''));
	...
	var List = view.container.extend({
		...
		afterBuild: function(){
			...
			// 操作菜单事件监听
			if (cfg.hasMenu){
				self.uiProxy(self.table, 'td[data-ctype="menu"]>a', 'click', 'eventClickMenu');
				pubjs.sync();
		...
		showLoading: function(opacity){
			...
			if (!opacity){
				...
				if (!mask){
					mask = self.loadingRow = $('<div class="M-tableListLoading"><i class="uk-icon-spinner uk-icon-spin"></div>').appendTo(el);
				}
		...
		eventSelectChange: function(evt, elm){
			...
			var sels = self.$selectedRowId[col.select_id] || [];
			var changed = false;
			var singleSelect = this.getConfig('singleSelect');
		...
		eventSelectRow : function(data,index,list){
			var c = list.getConfig();
			if(list.rows[index]){
				list.rows[index].find("input[data-select]:first").click();
				if(c.rowSelectOp){
					list.rows[index].find("[data-op]").click();
				}
			}
		},
		/**
		...
		onMenuSelected: function(ev) {
			...
			if (menuBtn) {
				index = +menuBtn.parents('tr').attr('data-index');
			}
		...
		eventClickMenu: function(ev, dom){
			var menu, cfg,
				...
				$td = $btn.parent(),
				data = self.getData(),
				index = +$td.parents('tr').attr('data-index');

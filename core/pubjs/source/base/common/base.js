define(function(require, exports){
	...
	var Scroller = pubjs.Module.extend({
		init: function(config, parent){
			...
			self.$config = config = pubjs.conf(config, {
				...
				'margin': 0, // 滚动条两端偏移位置支持数组形式分别制定两端偏移量
				'offset': 0, // 滚动条侧面偏移位置
				'width': 0, // 指定滚动容器宽度
				'height': 0, // 指定滚动容器高度
				'auto': true, // 自动隐藏
		...
		bindEvent: function(){
			...
			if (el.attachEvent){
				...
				ctr.addEventListener('touchstart', self, false);
			}
		},
		handleEvent: function(evt){
			...
			switch (evt.type){
				...
				case 'mousewheel':
					...
					if (self.$config.get('wheel') === 'noshift' && evt.shiftKey){
						return;
					}
					self.eventWheel(evt);
				break;
		 ...
		 * @param  {Boolean} silent 不发送消息
		 * @param  {Number} animate 动画时间，大于0时则执行动画
		 * @return {None}
		 */
		scrollBy:function(offset, silent, animate){
			var self = this;
			self.scrollTo(self.$info.pos - offset, silent, animate);
			return self;
		},
		/**
		 * 滚动到指定位置
		...
		eventMouseDown: function(evt){
			...
			if (!self.$mouse){
				...
				if (d.attachEvent){
					...
					// 移动端
					d.addEventListener('touchmove', self, false);
					d.addEventListener('touchend', self, false);
				}
			}
			self.$mouse = {
				screenX: evt.screenX,
		...
		eventMouseUp: function(evt){
			...
			if (d.detachEvent){
				...
				// 移动端
				d.removeEventListener('touchend', self, false);
				d.removeEventListener('touchmove', self, false);
			}
			self.eventMouseMove(evt);
	...
	var Pager = view.widget.extend({
		init: function(config, parent){
			config = pubjs.conf(config, {
				...
				'subClass': {
					'prev': 'btn btn-sm btn-default',
					'next': 'btn btn-sm btn-default',
					'first':'btn btn-sm btn-default',
					'last': 'btn btn-sm btn-default',
					'page': 'btn btn-sm btn-default',
					'active': 'M-commonPagerActive',
					'disable': 'M-commonPagerDisable',
					'goto': 'M-commonPagerGoto form-element',
					'jump': 'btn btn-sm btn-default'
				},
				'subText': {
					'prev': _T('<'),
					'next': _T('>'),
		...
		afterBuild: function(layout){
			...
			self.uiProxy(self.getDOM(), 'select', 'change', 'eventSelectChange');
		...
		updateUI: function(){
			...
			if (cfg.stepButton && self.count > cfg.bounds){
				if (!doms.stepPrev){
					con = doms.stepPrev = self.getContainer('stepPrev');
					doms.prev = $('<button data-type="prev"/>').addClass(cls.prev).appendTo(con);
				}
			}else if (doms.stepPrev){
				doms.stepPrev.hide();
			}
			// 前进
			if (cfg.stepButton && self.count > cfg.bounds){
				if (!doms.stepNext){
					con = doms.stepNext = self.getContainer('stepNext');
					doms.next = $('<button data-type="next"/>').addClass(cls.next).appendTo(con);
				}
			}else if (doms.stepNext){
				doms.stepNext.hide();
			}
			...
			if(cfg.showSizeTypes){
				con = doms.sizeTypeBox = self.getContainer('sizes')
								.html('<span class="">'+LANG(cfg.subText.prePage)+'</span>');

				util.each(cfg.sizeTypes, function(size){
					if (size){
						$('<button data-type="size" class="btn btn-default" />')
							.toggleClass(cls.active, size == self.size)
							.attr('data-size', size)
							.text(size).appendTo(con).hide();
					}
				});
				...
				if(!doms.select){
					...
					util.each(cfg.sizeTypes, function(size){
						if (size){
							...
							html.push('<option data-type="size" '+selected+' value ="'+size+'">'+size+'</option>');
						}
					});
					doms.select.append(html.join('')).val(self.size);
				}else if(doms.select){
					doms.select.appendTo(self.getContainer('sizes'));
				}
		...
		countPage: function(init){
			...
			if (init){
				...
				if(util.index(cfg.sizeTypes, cfg.size) === null){
					...
					cfg.sizeTypes.sort(function(a, b){
						return a - b;
					});
				}
			}
	...
	var Search = view.container.extend({
		init: function(config){
			config = pubjs.conf(config, {
				...
				'addClass': '', // simple 清爽版
				'inputClass': 'M-commonSearchInput form-element',
				'searchTip': LANG('请输入搜索内容'),
				'searchText': '',
				'width': 200,
				'height': null,
				'buttonClass': 'M-commonSearchDo',
				'buttonText': LANG(''),
				'undoClass': 'M-commonSearchUndo icon-close',
				'undoText': LANG(''),
		...
		afterBuild: function(){
			...
			// 创建输入框外包层
			doms.con = $('<div class="form-element-group M-commonSearchInputCon"/>').appendTo(el);

			if(cfg.width){
				doms.con.width(cfg.width);
			}
			...
			// 创建输入框
			doms.input = $('<input type="text"/>').attr({
			...
			if(cfg.height){
				doms.input.css('max-height', cfg.height);
			}

			// 创建取消按钮
			doms.undo = $('<span class=""/>').attr({
				'class': cfg.undoClass
			}).appendTo(doms.con);

			// 创建搜索按钮
			doms.button = $('<span class="form-element-extra arrow"/>').appendTo(doms.con);
			$('<i/>').attr({
				'class': cfg.buttonClass
				,'value': cfg.buttonText
			}).appendTo(doms.button);

			// 如果有多词搜索配置
			if(cfg.hasWordsSearch){
				doms.words = $([
					'<div class="M-commonSearchWords">',
						'<i class="triangle-top" title="'+LANG('多词搜索')+'"/>',
					'</div>'
				].join('')).appendTo(el);
				el.addClass('hasWords');
				self.uiBind(doms.words, 'click', 'eventShowWords');
			}


			self.uiBind(doms.input, 'keyup', 'eventKeyPress');
			self.uiBind(doms.button, 'click', 'eventButtonClick');
			self.uiBind(doms.undo, 'click', 'eventUndoClick');
		},
		/**
		 * 输入框监控回车的输入自动搜索
			...
			if (text != self.$data){
				...
				if(c.showResult){
					self.fire('search', {
						'type': 'search',
						'value': text,
						'desc': self.renderResultDesc(text.split(' '))
					});
				}else{
		...
		eventShowWords: function(evt, elm){
			var self = this;
			var tip = self.get('wordsSearch');
			if(!tip){
				tip = self.create('wordsSearch', WordsSearch, {
					'anchor': $(elm)
				});
			}
			tip.toggle();
		...
		reset: function(){
			...
			if(self.get('wordsSearch')){
				self.get('wordsSearch').reset();
			}
			return self;
		},
	...
	// 多词搜索弹层
	var WordsSearch = tip.tooltip.extend({
		init: function(config){
			config = pubjs.conf(config, {
				// 'anchor': '',
				'autoShow': true,
		...
		afterBuild: function(){
			...
			var el = self.getDOM();
			var con = $([
				'<div class="M-commonWordsSearch">',
					'<i class="icon-close"/>',
					'<div class="title">',
						LANG('多词搜索'),
					'</div>',
					'<div class="desc">',
						LANG('请输入关键词，不同关键词换行隔开'),
					'</div>',
					'<textarea/>',
			...
			self.$textarea = con.find('>textarea').select();
			self.$button = con.find('>button');
			self.$close = con.find('>i');

			self.uiBind(self.$textarea, 'keydown', 'eventEnterSearch');
			self.uiBind(self.$button, 'click', 'eventWordsSearch');
			self.uiBind(self.$close, 'click', 'eventClose');
			...
			self.uiBind(document, 'mouseup tap', function(ev, elm){
				// 跳过打开按钮
				if($(ev.target).parent().hasClass('M-commonSearchWords') || $(ev.target).hasClass('M-commonSearchWords')){
					return;
				}
		...
		eventEnterSearch: function(evt, elm){
			var self = this;
			// ctrl + enter 可确认
		...
		eventWordsSearch: function(evt, elm){
			var self = this;
			var val = self.$textarea.val().split(/[\r\n]+/g);
			util.each(val, function(key){
				key = util.trim(key);
				return key || null;
			});
			...
			var data = {
				'type': 'words', // 名称
				'value': val, // 结果
				'desc': val && val.length ? desc : '' // 描述语句
			}
			self.fire('wordsSearch', data);
			self.hide();
			return false;
		},
	...
	exports.wordsSearch = WordsSearch;
	...
	var SearchOld = view.container.extend({
		init: function(config){
			config = pubjs.conf(config, {
				...
				'buttonText': LANG('搜索'),
				'undoClass': 'btn btn-default M-commonSearchOldUndo',
				'undoText': LANG('取消')
			});
	...
	var ExcelExport = view.container.extend({
		...
		afterBuild: function(){
			var self = this;
			var c = self.getConfig();
			self.addClass('M-commonExcel M-HighGridExport');

			var btn = $('<em title="'+c['name']+'"/>');
			self.append(btn);
			self.uiBind(btn, 'click', 'eventButtonClick');

			// 自定义导出
			if (c.subs){
				btn = $('<button class="btn btn-default"><i class="uk-icon-caret-down" /></button>');
				self.append(btn);
	...
	***/
	...
	var Tab = view.container.extend({
		...
		afterBuild: function(){
			...
			for (var name in c.list){
				...
				if (type === 'button'){
					li = $('<button class="btn btn-default" />').text(tab.text);
				}else{
					li = $('<li/>').append($('<a class="text-truncate"/>').text(tab.text)).attr('title',tab.text);
				}
				// 项目放入头容器中
		...
		updateTitle: function(title, name){
			var self = this;
			self.$tabs[name].head.find('a').text(title);
			return self;
		},
		toggleTab: function(name, isShow){
			var self = this;
			if(!util.has(self.$tabs, name)) {
				return self;
			}

			if(isShow) {
				self.$tabs[name].head.show();
			}
			else {
				self.$tabs[name].head.hide();
			}
		}
	});
	exports.tab = Tab;
	...
	var LoadingMask = view.container.extend({
		...
		afterBuild: function(){
			var self = this;
			var c = self.getConfig();
			self.addClass('M-commonLoading').append('<i class="uk-icon-spinner anim-spin">');
		...
		hide: function(){
			this.getDOM().removeClass('noMask').hide();
			return this;
		},

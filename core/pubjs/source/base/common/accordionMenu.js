	...
	var Base = view.container.extend({
		...
		onData:function(err ,data){
			...
			if(data && data.items){
				...
				self.fire('accordionMenuDataLoaded',{data:data.items});

			} else {
				pubjs.error(LANG("无菜单数据或数据格式不正确"));
			}
		},
		...
		buildItems: function(items, target, level) {
			...
			// 子菜单需要隐藏
			if (level > 1 && self.getConfig('defaultCollapse')) {
				ulDom.hide();
			}
		...
		buildItem: function(item, menuContainer, index, level) {
			var	itemDom,
				...
				//有子级，必须小于限定层数
				itemHasSub = util.isArray(itemSub) && itemSub.length && ((!c.maxLevel || (c.maxLevel && level < c.maxLevel)));

			if(c.maxLevel && level > c.maxLevel){
				return false;
			}

			if (c['menu_html_key'] in item) {
				itemDom = $('<li class="M-accordionMenu-item"></li>').appendTo(menuContainer).data(item);//绑定item数据到此dom上
				if(!c.defaultCollapse){
					itemDom.addClass('M-accordionMenu-open');
				}
		...
		eventMenuToggle: function(ev) {
			...
			self.getContainer().find('.M-accordionMenu-item').removeClass(self.getConfig('currentClass'));
			itemDom.addClass(self.getConfig('currentClass'));
			...
			self.fire('menuItemClick',{data:itemDom.data()|| {}});

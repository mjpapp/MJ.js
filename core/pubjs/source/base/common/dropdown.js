...
define(function(require, exports){
	...
	var DropdownBase = view.container.extend({
		...
		afterBuild: function(){
			...
			var el = self.getDOM();
			var doms = self.$doms = {};
			var con = doms.resultCon = $('<div class="form-element-group"/>').appendTo(el);

			self.addClass('M-commonDropdown');
			doms.result = $('<div class="form-element result text-truncate"/>').appendTo(con);
			doms.arrow = $('<div class="form-element-extra arrow"><i class="triangle-top"/></div>').appendTo(con);
			...
			doms.selectCon = $('<div />').appendTo(el);
		...
		hideOption: function(){
			...
			self.$show_option = false;
			$(document.body).unbind('mouseup.dropdown tap.dropdown');
		...
		showOption: function(){
			...
			switch (c.appendTo){
				...
				case 'popup':
					...
					self.$doms.list.css({
						top: offset.top + el.outerHeight() + c.offsetTop + popOffset.top,
						left: offset.left + c.offsetLeft + popOffset.left,
						'z-index': c['z-index'] || 1001
					}).appendTo($('#SCENES_POPUP'));
					break;
				default:
					self.$doms.list.css({
						top: c.height + c.offsetTop,
						left: 0 + c.offsetLeft
					});
			}
			...
			self.$doms.list.show();
			self.showSelect();
			self.uiBind(document.body, 'mouseup.dropdown tap.dropdown', 'hideOption');
		...
		eventSearch: function(evt){
			...
			var visible_list = doms.options.find('>a.option:visible').removeClass('highlight'); // 可见的list组
			...
			switch (evt.keyCode){
				...
				case 38:							// 向上方向键
					...
					if(self.$scrollIndex < 0){
						self.$scrollIndex = max -1;
					}
					self.move(visible_list, 'up');
					break;
				case 40:							// 向下方向键
					self.$scrollIndex++;

					// 向下已到底部
					if(self.$scrollIndex >= max){
						self.$scrollIndex = 0;
					}
				...
				case 13:							// 回车键
					...
					// 模拟点击
					self.eventSelect(null, visible_list.eq(this.$scrollIndex));
					break;
				default:
		...
		move: function(dom){
			...
			if(c.search){
				search_height = doms.search && doms.search.outerHeight() || 0;
			}

			// 系数
			var index = Math.floor((option_height - search_height)/height)  || 2;

			// 滚动条跟随移动, -1是修正数
			self.$.scroll.scrollTo(-(self.$scrollIndex - (index - 1)) * height);
			// 按键选中状态
			dom.eq(self.$scrollIndex).addClass('highlight');
		},
		// 显示或者隐藏搜索按钮
		...
		buildList: function(){
			...
			//如果不是创建在popup里面，则将选项创建在selectCon中
			doms.list = $('<div class="M-commonDropdownList"/>').appendTo(doms.selectCon);

			self.uiBind(doms.list, 'mouseup tap', 'eventTrackMe');
			doms.list.css('min-width', c.option_width || c.width);

			if (c.search){
				doms.search = $('<div class="search" />').appendTo(doms.list);
				doms.search_key = $('<input type="text" />').appendTo(doms.search).attr('placeholder', LANG('请输入搜索内容'));
				doms.btnSearch = $('<div class="btnSearch"/>').appendTo(doms.search);
				doms.btnCancel = $('<div class="btnCancel"/>').appendTo(doms.search);
				self.uiBind(doms.search_key, 'mouseup tap', 'eventTrackMe');
				...
				if(util.isMobile()){
					self.$watchMobileInput = setInterval(function(){
						if(self.$lastSearchVal != doms.search_key.val()){
							self.eventSearch({});
						}
					}, 300);
				}
			...
			if (c.scroll){
				...
				if(util.isMobile()){
					// self.showScroll(doms.scroll, doms.options, {
					// 	autohidemode: true
					// });
					doms.scroll.css('overflow-y', 'auto');
				}
			}else {
				self.buildPanel();
			}
		...
		buildOption: function(opt, id){
			...
			var dom = $('<a class="option text-truncate" href="#"/>').appendTo(self.$doms.options).attr('title', name);
			if (opt.nonData){
	...
	var SubDropdownList = DropdownBase.extend({
		...
		buildSubPanel: function(opt, id, parent_elm){
			...
			var panel = $([
				'<div class="subs" >',
					'<div class="subsCon"/>',
				'</div>'
			].join('')).appendTo(doms.subOption);
			panel.width(opt.option_width || c.option_width || c.width);
			parent_elm.attr('data-subs', sid).addClass('has-sub').append($('<i class="uk-icon-angle-right"/>'));
			...
			c = self.$subs_opts[sid] = {
				...
				'options': opt[c.skey],
				'list': $('<div class="options" data-type="sub"/>').appendTo(panel.find('.subsCon')),
				'arrow': $('<div class="sub_arrow"/>').appendTo(panel),
				'con': panel.find('.subsCon')
			};
			...
			if (opt.scroll){
				...
				// });
				// if(util.isMobile()){
				// 	// self.showScroll(c.container, c.list, {
				// 	// 	autohidemode: true
				// 	// });
				// 	c.container.css('overflow-y', 'auto');
				// }
			}

			if(config.sub_height && c.options.length*config.height > config.sub_height){
				panel.find('.subsCon').height(config.sub_height);
				// if(util.isMobile()){
				// 	c.con.css('overflow-y', 'auto');
				// }else{
					c.scroll = self.create('scroller' + sid, common.scroller, {
						'target': c.con,
						'content': c.list,
						'watch': 200,
						'dir': 'V'
		...
		eventSelect: function(evt, elm){
			...
			if (!c.canSelectParent && a.attr('data-subs')){
		...
		showSubOption: function(sid, force){
			...
			// 新的定位方式-------------------------------------------start
			// var el = self.getDOM();
			var offset = util.offset(anchor, doms.list);
			var w = anchor.outerWidth();

			// 获取滚动条宽度
			var scroll = anchor.parent().next('.M-commonScroller');
			var scrollWidth = 0;
			...
			diff = 0;
			if(opts[sid].options.length > 1){
				diff = con.height()/2 - c.height/2;
			}

			// 移动端位差, 原生offsetTop取值不同所致
			var mobile_diff = 0;
			if(util.isMobile()){
				var ul = anchor.parent('div');

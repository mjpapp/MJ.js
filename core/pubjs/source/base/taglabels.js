define(function(require, exports){
	...
	var TagLabels = view.container.extend({
		...
		afterBuild: function(){
			...
			var el = self.$checkDOM = this.getDOM();

			$('<label class="M-tagLabelsLabel"/>').text(c.label).appendTo(el).toggle(c.hasLabel);
			...
			if(!c.hasLabel){
				this.tagLabelsContainer.getDOM().find('.M-tagLabelsInner')
					.toggleClass('M-tagLabelsInnerNoLable', !c.hasLabel);
			}
		...
		,refresh:function(){
			var c = this.getConfig();
			if (!util.checkUI(this.$checkDOM, 'refresh', this)){
				return this;
			}

	...
	var BaseNoDate = view.widget.extend({
		init: function(config, parent){
			config = pubjs.conf(config, {
				...
				'sort': 'pageviews',	// 排序字段
				'order': 'desc',		// 排序顺序
				'order_field': 'order',	// 顺序排序字段sort|-1
				...
				'layout': {
					...
					'rows': [
						'sidebarFilterResult',
						'search(.pb5 mr10)',
						'sidebarFilterBtn(.pb5 mr10)',
						'sidebarFilterCon',
						'tab(auto)|export(*right)(.mr10)(auto)(.pb5)|refresh(*right)(auto)(.pb5)',
						'amount',
						'list',
						'pager(.tr)(.pt10)'
					]
			...
			self.updateMetricCols(config);

			self.Super('init', arguments);
		},
		updateMetricCols: function(config, field) {
			var c = config.get();
		...
		afterBuild: function(layout){
			...
			if (cfg.hasSearch){
				self.createDelay(
					...
					{'target': self.getContainer('search'),'width': 200}
				);
				self.getContainer('search').parent().addClass('fl');
			}else {
				layout.removeCol('search');
			}

			if (cfg.hasFilter){
				var sidebarFilterBtn = self.getContainer('sidebarFilterBtn').addClass('fl M-gridFilterSidebarBtn');
				var sidebarFilterCon = self.getContainer('sidebarFilterCon').addClass('M-gridFilterSidebarCon M-HighGridFilterSidebar');
				var sidebarFilterResult = self.getContainer('sidebarFilterResult');
				self.createDelay(
					'sidebarFilter', '@base/highGrid.filter',
					{
						'target': sidebarFilterBtn,
						'hasFilterSidebar': cfg.hasFilterSidebar,
						'target_filterSidebar': sidebarFilterCon,
						'parentTarget': self.$checkDOM,
						'sidebar_config': cfg.sidebar_config,
						'hasFilterResult': cfg.hasFilterResult,
						'target_result': sidebarFilterResult,
						'isOldGrid': true // 老表格
					}
				);
				sidebarFilterBtn.parent().addClass('fl pr');
				sidebarFilterCon.parent().addClass('fl');
			}else {
				self.getContainer('sidebarFilterBtn').parent().remove();
				layout.removeCol('sidebarFilterBtn');
				layout.removeCol('sidebarFilterCon');
				layout.removeCol('sidebarFilterResult');
			}
			...
			if (cfg.hasRefresh){
				...
				div.html([
					'<span data-type="0" class="M-gridRefreshAuto" ><i class="uk-icon-square"></i>'+LANG("自动刷新")+'</span>',
					'<button title="'+LANG('刷新报表')+'" class="refNormal"><i class="uk-icon-rotate-right"/></button>'
				].join(''));
				var ref = self.$refresh = {
					dom: div,
					check: div.find('.M-gridRefreshAuto'),
					button: div.find('button')
				};
				self.refreshCallBack = self.refreshCallBack.bind(self);
				if (cfg.refresh_auto){
					ref.check.find('i').addClass('act uk-icon-check-square');
					ref.check.attr('data-type', 1);
			...
			self.createDelay(true, function(){
				...
				// 更新排序记录
				param[cfg.order_field] = list.getSort();
				// 更新分页设置
				if (cs.pager){
				...
				if(cfg.singleSelect){
					//隐藏序号隔离全选的checkbox
					this.getDOM().find('input[data-type="select_all"]').hide();
				}
			});
		},
		...
		eventRefreshManual: function(){
			this.$refresh.button.find('i').addClass('refing uk-icon-spin uk-icon-spinner');
			this.load(true);
		},
		toggleRefresh: function(mode){
			...
			self.$refresh.check
				...
				.find('i').toggleClass("uk-icon-check-square",mode);
		...
		load: function(auto){
			...
			switch(cfg.reqType){
				...
				break;
				case 'websocket':
					self.$reqID = pubjs.mc.send(cfg.url, param, self.onData.bind(self));
				break;
			}
		...
		onData: function(err, data, param){
			...
			// 自动刷新
			if (self.$refresh){
				self.$refresh.button.prop('disabled', false).find('i').removeClass('refing uk-icon-spin uk-icon-spinner');
				if (c.refresh_auto){
					self._toggleRefresh(1);
					// 自动拉取时, 错误不更新不提示错误
					if (err){ return; }
		...
		onChangeSort: function(ev){
			var cfg = this.getConfig();
			this.$sys_param[cfg.order_field] = this.$.list.getSort();
			this.load();
			return false;
		},
		/**
		...
		,getSelectRowIds:function(name){
			return this.$.list.getSelectRowIds(name);
		}

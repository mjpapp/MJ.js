define(function(require, exports){
	...
	var Base = view.container.extend({
		...
		afterBuild: function(){
			...
			self.$doms = {
				...
				'arrow': $('<div class="M-tipArrow triangle" />').toggle(!!c.hasArrow).appendTo(el)
			};
		...
		doHide: function(){
			...
			self.setTimeout('realHide', self.getConfig('hideDuration'));
			return self;
		},
		realHide: function(){
			if (!this.$isShow){
		...
		updatePosition: function(){
			...
			if (self.$visible){
				...
				self.css({
					'top': pos.top + popOffset.top,
					'left': pos.left + popOffset.left
				});
			}
		},
		// 显示设置
	...
	var Tooltip = Base.extend({
		...
		,afterBuild: function(){
			...
			el.find('.M-tipArrow').removeClass('triangle').addClass('custom');
		}
		,realHide: function(){
			if (!this.$isShow){
				this.css('display', 'none');
		...
		,show: function(){
			...
			// 更新三角形的位置
			if(c.arrowAlign){
				self.$doms.arrow.addClass(c.arrowAlign).css(c.arrowAlign, c.arrowAlignPixel);
			}
	...
	var Description = view.container.extend({
		...
		eventToggleTip: function(ev){
			...
			if(!mod){
				mod = this.create('tip', Base, util.extend({
					anchor: this.getDOM(),
					width: c.width,
					height: c.height
				}, c.tip_config));

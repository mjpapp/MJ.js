define(function(require,exports){
	...
	var Base = view.container.extend({
		...
		delayBuild: function() {
			...
			$([
				'<div class="G-framePopHead">',
					'<div class="G-framePopHeadProgress b-is-danger"/>',
				...
				'</div>',
				'<div class="G-framePopBoard"></div>'
			].join('')).appendTo(el);

			var mask = $('<div class="G-framePopMask"/>').insertBefore(el);

			var doms = self.$doms = {
				head: el.find('.G-framePopHead'),
				headTitle: el.find('.G-framePopHeadTitle'),
				headExit: el.find('.G-framePopHeadExit'),
				headProgress: el.find('.G-framePopHeadProgress'),
			...
			self.uiBind(doms.mask, 'click tap', 'eventHide');
		},
		// 增加一项
		addItem: function(opts) {
			...
			if (!cont){
				...
				util.each(modulesConfig, function(modconf) {
					...
					// 同步时间数据，用于需要同步时间的表格
					var syncDate = modconf.config && modconf.config.syncDateParam;
					var parentGridDateParam = opts.source.dateParam;
					...
					container.createBusiness(
						...
						util.extend(
							...
							(syncDate ? {syncDateParam: parentGridDateParam} : {}),
							{
						...
						function(mod) {
							...
							// source的下级subSource
							subSource['container_name'] = name;
							opts.source.subSource = subSource;
							// 更新层级
							self.updateCrumbLevel(opts.source, mod);

							self.updateProgress('over');
						}
					);
				});
		...
		// 激活某一项
		showItem: function(name, updateTitle) {
			var self = this;
			var list = self.$containers;
			var title = '';
			...
			if(updateTitle && self.get('crumb')){
				self.get('crumb').updateSubTitle(title);
			}
			// 发送show广播通知
			self.cast('popShow');
		},
		...
		show: function(){
			...
			self.updatePosition();
			self.toggleShowClass(true);

			// 发送show广播通知
			pubjs.core.cast('popShow');
			self.Super('show', arguments);

			return self;
		},
		// 移除show样式
		toggleShowClass: function(bool){
			var self = this;
			self.getDOM().toggleClass('show', bool);
			self.$doms.mask.toggleClass('show', bool);
			return self;
		},
		...
		updatePosition: function(){
			...
			if(util.isMobile()){
				el.css({
					top: 0,
					height: $('.G-frameBodyMenu').outerHeight()
				});
			}else{
				el.css({
					top: doc.scrollTop()
		...
		// 更新加载进度
		updateProgress: function(over){
			var self = this;
			var doms = self.$doms;

			if(over){
				clearInterval(self.$progressTimeId);
				doms.headProgress.width('100%');

				setTimeout(function(){
					doms.headProgress.hide();
				}, 1000);
				return self;
			}

			doms.headProgress.width(0).show();
			self.$progressTimeId = setInterval(function(){
				var width = doms.headProgress.width();
				if(width <= 800){
					doms.headProgress.width(width + 10);
				}
			}, 10);
		...
		//-----------------------有关crumb模块方法
		// 更新面包屑层级
		updateCrumbLevel: function(opt, mod){
			var self = this;
			var crumb = self.get('crumb');
			if(crumb && opt){
				// 暂时对比报表可点击下拉drop
				crumb.updateDropEvent(opt.popType=='compare' ? true : false);
				// 更新层级
				crumb.updateLevel(opt);
				// 更新对应的子表格模块
				crumb.updateSubGridMod(mod);
			}
			return self;
		},
		...
		onCrumbChangeLevel: function(ev){
			var self = this;
			self.showItem(ev.param, true);
		}
	});
	exports.base = Base;
	...
	var Crumb = view.container.extend({
		init: function(config) {
			config = pubjs.conf(config, {
				...
				'periodOptions': [
					{id: 'hour', name: LANG('小时'), value: 0, 'disabled': true},
					{id: 'date', name: LANG('天'), value: 1, 'disabled': true},
					{id: 'week', name: LANG('周'), value: 2, 'disabled': true},
					{id: 'month', name: LANG('月'), value: 4, 'disabled': true}
				]
			});
			...
			if(levelModule){
				...
				if(config && left && right && rightPeriod){
					...
					if(isSubSource){
						...
						self.uiUnbind(left.getDOM(), 'click', 'eventChangeLevel');
						// 再绑定
						self.uiBind(levelModule['cancel'], 'click', config, 'eventCancelLevel');
						self.uiBind(left.getDOM(), 'click', config, 'eventChangeLevel');
					}
		...
		// 通过ids或者已选data
		getSelectsData: function(config, key){
			var self = this;
			var selectsData = self.$selectsData = [];

			if(config){
				if(config.selects.length && config.gridData){
					var list;
					if(config.gridData.items){
						list = config.gridData.items;
					}else{
						list = config.gridData;
					}
					util.each(config.selects, function(item){
						var data = util.find(list, item, key || '_id');
						if(data){
							selectsData.push(data);
						}
					});
				}
			}
			return self.$selectsData;
		},
		// 更新子标题
		...
		// 更新drop是否可点击
		updateDropEvent: function(bool){
			var self = this;
			util.each(self.$level, function(item){
				if(item){
					if(item['right']){
						item['right'].toggleEvent(bool);
					}
					if(item['rightPeriod']){
						item['rightPeriod'].toggleEvent(bool);
					}
				}
			});
			return self;
		...
		// 获取对应的subGrid模块
		getSubGridMod: function(){
			return this.$subGridMod || null;
		},
		// 构建层级
		...
		eventChangeLevel: function(evt, elm){
			if(evt && evt.data){
				var self = this;
				self.fire('crumbChangeLevel', evt.data.container_name);
				this.getDOM().find('.M-drop').removeClass('blue');
				$(elm).addClass('blue');
			}
		},
		// drop改变事件汇总

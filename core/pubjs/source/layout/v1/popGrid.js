define(function(require,exports){
	...
	var Menu = view.container.extend({
		...
		afterBuild: function() {
			...
			var html = $([
				'<div class="G-framePopGridMenu-main">',
					'<ul class="G-framePopGridMenu-content"></ul>',
				'</div>'
			].join('')).appendTo(container);

			doms = self.$doms = {
				wrap: html,
				main: $('.G-framePopGridMenu-main', container),
				content: $('.G-framePopGridMenu-content', container),
				group: {}
			};
		...
		// 添加一项
		addItem: function(key, text, fixed, opts) {
			var self = this;
			var doms = self.$doms;
			var data = self.$data;
			...
			var groupKey = group.name + group.id;	// 分组key

			var name = group.text + (group.id ? ('|' + group.id) : '');
			if(!doms.group[groupKey]){
				doms.group[groupKey] = $([
					'<li class="G-framePopGridMenu-group">',
						'<div class="G-framePopGridMenu-group-label text-truncate" title="'+ name +'">',
							'<span>',
								name,
							'</span>',
						'</div>',
						'<ul class="G-framePopGridMenu-group-item">',
						'</ul>',
					'</li>'
				].join('')).appendTo(doms.content);
			}else{
				doms.group[groupKey].show();
			}

			var item = $([
				'<li class="G-framePopGridMenu-item" data-key="' + key + '">',
					'<span class="G-framePopGridMenu-item-corner"/>',
					'<span class="G-framePopGridMenu-item-content">' + text + '</span>',
					'<span class="G-framePopGridMenu-item-ctrl">',
						'<span class="G-framePopGridMenu-item-ctrl-wrap clear">',
							'<span class="G-framePopGridMenu-item-removeBtn fr"><i class="uk-icon-times"></i></span>',
							'<span class="G-framePopGridMenu-item-fixedBtn fl"><i></i></span>',
						'</span>',
					'</span>',
				'</li>'
				].join('')).appendTo(doms.group[groupKey].find('ul'));
			var content = $('.G-framePopGridMenu-item-content', item);
			var removeBtn = $('.G-framePopGridMenu-item-removeBtn', item);
			var fixedBtn = $('.G-framePopGridMenu-item-fixedBtn', item);
			var ctrl = $('.G-framePopGridMenu-item-ctrl', item);
	...
	var Base = view.container.extend({
		init: function(config) {
			config = pubjs.conf(config, {
				'max': 40,         // 最多允许存在的表格最大数
				'class': 'G-framePopGridBlock container'
			});
		...
		afterBuild: function() {
			...
			$([
				'<div class="G-framePopGridMain">',
					'<div class="G-framePopGridMenu"></div>',
					'<div class="G-framePopGridContent"></div>',
				'</div>',
				'<div class="G-framePopGridHead">',
					'<div class="exit">',
						'<span>'+LANG('可使用快捷键 "ESC"')+'</span>',
						'<i class="icon-close"/>',
					'</div>',
				'</div>',
				'<div class="G-framePopGridCtrl">',
					'<div class="G-framePopGridCtrlBtn">',
			...
			doms = self.$doms = {
				ctrl: $('.G-framePopGridCtrl', container),
				main: $('.G-framePopGridMain', container),
				menu: $('.G-framePopGridMenu', container),
				content: $('.G-framePopGridContent', container)
			};

			self.uiBind(doms.ctrl, 'click', 'eventCtrlClick');
			self.uiBind(container.find('.G-framePopGridHead i'), 'click', 'eventCtrlClick');
		...
		addItem: function(opts) {
			...
			if (!cont){
				...
				list.push(cont);
				self.$.menu.addItem(name, title, cont.fixed, opts);
